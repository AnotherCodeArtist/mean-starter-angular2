...
const createSamplePOIs =(nr,user) =>
  Array.from(Array(nr).keys())
    .reduce((promise,number) => promise.then((array)=>
        createSamplePOI(number+1,user).then(poi => {array.push(poi);return array;}))
      ,Promise.resolve([]));
  ...
  it("should be possible to delete a poi, if you are the creator or an admin", done => {
    ...
    createTestUser()
      ...
      .then(res => {
        res.should.have.status(200);
        let poi = res.body;
        poi.name.should.be.equal(poiToDelete.name);
        poi.description.should.be.equal(poiToDelete.description);
        should.exist(poi._id);
        poi.loc.coordinates.should.be.an('array').and.have.lengthOf(2);
        poi.loc.coordinates[0].should.be.equal(poiToDelete.loc.coordinates[0]).and.be.a.Number;
        poi.loc.coordinates[1].should.be.equal(poiToDelete.loc.coordinates[1]).and.be.a.Number;
        should.exist(poi.creator);
        poi.creator.local.username.should.be.equal("johnny");
